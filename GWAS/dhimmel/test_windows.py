...
# Lead SNP file generated by test_leadSNP.py 
...
# Proxy SNP file generated by SNAP search engine, manually. 
...
url_proxy_snp = join(
    datasets_url,
    'gwas/dhimmel/manual/do-slim-proxy-SNPs.tsv'
    )
...
def download(remote_path=None, download_dir=None, force=False):
    ...
    if exists(save_path):
        st_size = os.stat(save_path).st_size
    else: 
        st_size = 0 

    if force==True or st_size==0: 
        os.system('wget -O %s %s' % (save_path, remote_path))
    return save_path


def load_proxySNP():
    localfile = download(remote_path=url_proxy_snp, 
        download_dir=join(test_root_dir, 'data/snap'))

    snap_df = pd.read_table(localfile, na_values=['N/A'])
...
def load_window(): 
    ...
    if exists(out_windows) and exists(chk_snap_errors):
        window_df = pd.read_csv(out_windows, sep='\t')
        ...
        window_df.to_csv(out_windows, index=False, sep='\t', 
            float_format='%.5g')
    # if
...
def test_coverage():
    ...
    # 머 ... 차이가 나는 것은 어찌보면 당연한 것 아닌가. 
    from hetio.preproc.GWAS.downloaded_201711 import test_gwas as gwas 
